String myIp="192.168.0.185";
int myPort=9999;
///////////////////////////
String otherIp="192.168.0.185";
int otherPort=9999;
///////////////////////////
Server myServer;
Client myClient;
Client otherClient;
byte[]dataInpBuffer=new byte[2];//[0]:piece@position [1]:movesTo. [2]:-1
byte[]dataOutBuffer=new byte[2];
...
int readIncomingData() {
  ...
  if (team==1) {
    ...
    if (myClient!=null) {
      ...
      println("incomingData:"+(int)dataInpBuffer[0]+" : "+(int)dataInpBuffer[1]);
    }
  } 
  if (team==0) {
